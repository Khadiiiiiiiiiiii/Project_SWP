        ...
        try ( Connection conn = DBContext.getConnection();  PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try ( ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    throw new SQLException("Không tìm thấy nhân viên với ID: " + staffId);
                }
            }
        }
        return staff;
    ...
    public void updateStaff(Staff staff, boolean updatePassword) throws SQLException {
        ...
        try ( Connection conn = DBContext.getConnection();  PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rowsUpdated <= 0) {
                ...
                throw new SQLException("Không thể cập nhật thông tin nhân viên với ID: " + staff.getStaff_id());
            }
    ...
    public void deleteStaff(int staffId) throws SQLException {
        ...
        try {
            ...
            if (staffRowsDeleted <= 0) {
                ...
                conn.rollback();
                throw new SQLException("Không thể xóa nhân viên từ bảng Staff với ID: " + staffId);
            }
            ...
            if (userRowsDeleted <= 0) {
                ...
                conn.rollback();
                throw new SQLException("Không thể xóa thông tin người dùng từ bảng Users với ID: " + userId);
            }
